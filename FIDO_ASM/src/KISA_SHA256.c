...
void SHA256_Process( OUT SHA256_INFO *Info, IN const unsigned char *pszMessage, IN unsigned int uDataLen )
{
	...
	{
		memcpy((unsigned char*)Info->szBuffer, pszMessage, (int)SHA256_DIGEST_BLOCKLEN);
		SHA256_Transform((unsigned long*)Info->szBuffer, (unsigned long*)Info->uChainVar);
...
void SHA256_Close( OUT SHA256_INFO *Info, IN unsigned char *pszDigest )
{
	...
	{
		memset((unsigned char*)Info->szBuffer + Index, 0, (int)(SHA256_DIGEST_BLOCKLEN - Index));
		SHA256_Transform((unsigned long*)Info->szBuffer, (unsigned long*)Info->uChainVar);
		memset((unsigned char*)Info->szBuffer, 0, (int)SHA256_DIGEST_BLOCKLEN - 8);
	}
	else
		memset((unsigned char*)Info->szBuffer + Index, 0, (int)(SHA256_DIGEST_BLOCKLEN - Index - 8));
